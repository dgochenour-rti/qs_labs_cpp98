...
extern "C" int publisher_main(int domainId, int sample_count)
{
    ...
    qs::qs_typeDataWriter * qs_type_writer = NULL;
    qs::qs_type *instance = NULL;
    ReturnCode_t retcode;
    InstanceHandle_t instance_handle = HANDLE_NIL;
    const char *type_name = NULL;
    int count = 0;  

    // LAB #8 - configure writer for 1Hz
    ...
    qs_type_writer = qs::qs_typeDataWriter::narrow(writer);
    if (qs_type_writer == NULL) {
    ...
    instance = qs::qs_typeTypeSupport::create_data();
    if (instance == NULL) {
        printf("qs::qs_typeTypeSupport::create_data error\n");
        publisher_shutdown(participant);
        return -1;
    }
    ...
    /*
    instance_handle = qs_type_writer->register_instance(*instance);
    */

    /* Main loop */
    for (count=0; (sample_count == 0) || (count < sample_count); ++count) {

        printf("Writing qs::qs_type, count %d\n", count);
        ...
        if (count % 2 == 0) {
            instance->name = "Cesar Chavez";
        } else {
            instance->name = "Frederick Douglass";
        }
        instance->id = 9017;
        instance->value1 = count;

        retcode = qs_type_writer->write(*instance, instance_handle);
        if (retcode != RETCODE_OK) {
            printf("write error %d\n", retcode);
        }

        // LAB #8 - wait for acks 
        qs_type_writer->wait_for_acknowledgments(DDS::DURATION_INFINITE);
        
        NDDSUtility::sleep(send_period);
    }

    /*
    retcode = qs_type_writer->unregister_instance(
    ...
    */
    ...
    /* Delete data sample */
    retcode = qs::qs_typeTypeSupport::delete_data(instance);
    if (retcode != RETCODE_OK) {
        printf("qs::qs_typeTypeSupport::delete_data error %d\n", retcode);
    }
